@PnCMultipleSubject
Feature: PnC Scenarios multiple subjects
  ...
  Scenario Outline: Create Subject/course
    ...
    And I fill the mandatory details for course or subject of "<type>","<Department>","<Min Marks>","<Max Marks>","<course type>" for creating "<number of courses>"
    Then I can see the successfull alert

    Examples: 
      | Modulename | Module             | Action | type  | Department | Min Marks | Max Marks | course type    | number of courses |
     #| Subjects   | Grades & Subjects  | Add    | Marks | Academic Adv Grp |  35 |       100 |Subject Section | 							4		 |
      | Courses    | Programs & Courses | Add    | Marks | Academic   |        35 |       100 | Course Section |                 4 |
  ...
  Scenario Outline: Add Grade/Program details
    ...
    Then I can see the successfull alert
    #And I click ok for session restart
    #And I refresh page
    Examples: 
      ...
      | Programs   | Programs & Courses | Add    | Manually   | Under Graduate | BTech    | Marks     | Academic Affairs | DIT-Dehradun EM test data1 |    10001111 |
  ...
  Scenario Outline: Create Prg or Grade intake details
    ...
    Examples: 
      | GradePeriodType | IntakeYear | AssignCalender | CalculationRule              | Action |
  ...
  Scenario Outline: Grade or Program subjects creation
    Given I click on grade or program periods more details
    And I click on "<BatchDetails>" in grade intake details
    And I Fill the course details with "<name>"
    Then I can see the successfull alert
  ...
  @MapPeriod
  Scenario: Map period for the subject or course
    Given I select the courses
    And I map the multiple subject with period
    Then I can see the successfull alert
    And I refresh page

  @mangesubjectsclass
  Scenario Outline: Manage subjects class or course section creation
    Given I click on "<Module>"
    And I click on "<Modulename>" Module
    And I Click on "<AcademicTerm>" and "<MarksorGradeType>"in manage subject class

    Examples: 
      | Modulename            | Module             | AssignCalender | AcademicTerm | MarksorGradeType |
      # | Manage Subject Class  | Grades & Subjects  | Crawford Lonehill College Cal |         2023 | Marks            |
      | Manage Course Section | Programs & Courses | Org Calendar   |         2023 | Marks            |

  @addsectionorclass
  Scenario: Add class or section
    Given I Click on add class and AssignCalender fill details for multiple subjects
    Then I can see the course parameterization alert

  @mapgradeperiod
  Scenario: Manage course section Map grade period creation
    Given I Click on map grade period for multiple courses
    Then I can see the successfull alert
    And I refresh page
  ...
  Scenario Outline: Manage subjects class creation
    ...
    And I click on "<BatchDetails>" in grade intake details
    And I click on add and select multiple subjects "<teachername>"
    Then I can see the successfull alert

    Examples: 
      | Modulename | Module             | Buttonname | BatchDetails      | teachername |
      # | Grades     | Grades & Subjects  | Search     | Subject Parameters |
      | Programs   | Programs & Courses | Search     | Course Parameters | Pallavi     |
